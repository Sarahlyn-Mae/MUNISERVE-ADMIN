...
function App() {
  ...
  const fetchMonthlyStatusData = async () => {
    const monthlyStatusData = await getMonthlyStatusData();
    setMonthlyStatusData(monthlyStatusData);
  };

  const getMonthlyStatusData = async () => {
    try {
  ...
  const getYearlyDataForBarangay = async () => {
    ...
    const collectionQueries = [
      ...
      query(
        collection(db, "birth_reg"),
        where("createdAt", ">=", Timestamp.fromDate(new Date(`${currentYear}-01-01`))),
        where("createdAt", "<=", Timestamp.fromDate(new Date(`${currentYear}-12-31`)))
      ),
      query(
        collection(db, "marriageCert"),
        where("createdAt", ">=", Timestamp.fromDate(new Date(`${currentYear}-01-01`))),
      ...
      <div className="container">
        ...
        <div className="reports">
          ...
          <div className="report">
            ...
            <div className="selected-date">
              <Chart
                options={chartData.selectedDate.options}
                series={chartData.selectedDate.series}
                type="radialBar"
                height="200"
              />
            </div>
        ...
        <div className="analytics">
          <div className="monthly-status-chart">
            <h2>Total Transactions based on Status per Month</h2>
            <ReactApexChart
              options={{
                chart: {
                  type: "bar",
        ...
        <div className="monthly-chart">
          <h2>Total Transactions Per Month</h2>
          <ReactApexChart
            options={{
              chart: {
                type: "bar",
        ...
        <div className="yearly-barangay-chart">
          <h2>Total Transactions Per Year Based on Barangay</h2>
          <ReactApexChart
            options={{
              chart: {
                type: "bar",
        ...
        {/* Display monthly data for the selected barangay */}
        {selectedBarangay && (
          <div className="monthly-chart-for-barangay">
            <h2>Total Transactions Per Month for {selectedBarangay}</h2>
            <ReactApexChart
              options={{
                chart: {
                  type: "bar",
                ...
                xaxis: {
                  categories: monthlyDataForBarangay.map((data) => data.month),
                },
              }}
              series={[
                {
                  name: "Count",
                  data: monthlyDataForBarangay.map((data) => data.count),
                },
              ]}
              type="bar"
              width={500}
